"""
GEX Level Calculator v2.4 - FIXED ZERO GAMMA
...
MIN_GEX_THRESHOLD = 100000  # Minimum GEX to consider a strike "active" (100K)
...
def calculate_gex_levels(data):
    ...
    """
    print("\n" + "="*60)
    print("CALCULATING GEX LEVELS - v2.4 (FIXED ZERO GAMMA)")
    print("="*60)
...
def interpolate_zero_gamma_fixed(strikes_data, underlying_price):
    """
    FIXED VERSION: Find meaningful zero gamma crossing.
    
    Key fixes:
    1. Ignore "dead zones" with near-zero GEX
    2. Find crossing closest to current price
    3. Only consider strikes with significant activity
    """
    print("\n🔧 Finding Zero Gamma (FIXED algorithm)...")
    
    # Calculate net GEX for each strike, filter out dead zones
    net_gex_by_strike = []
    for strike in sorted(strikes_data.keys()):
        ...
        # Only include strikes with significant GEX (not dead zones)
        if abs(net_gex) >= MIN_GEX_THRESHOLD:
            net_gex_by_strike.append((strike, net_gex))
    
    print(f"   Active strikes (|GEX| ≥ {MIN_GEX_THRESHOLD:,}): {len(net_gex_by_strike)}")
    
    if len(net_gex_by_strike) < 2:
        print(f"   ⚠️  Insufficient active strikes, using current price")
        return underlying_price
    
    # Find ALL meaningful zero crossings
    crossings = []
    ...
    for i in range(len(net_gex_by_strike) - 1):
        ...
        if (gex1 <= 0 <= gex2) or (gex2 <= 0 <= gex1):
            ...
            distance_from_price = abs(zero_gamma - underlying_price)
    ...
    if not crossings:
        print("   ❌ No zero crossings found in active strikes")
        return underlying_price
    
    # Select the crossing CLOSEST to current price
    best_crossing = min(crossings, key=lambda x: x['distance'])
    
    print(f"   ✅ Found {len(crossings)} meaningful crossing(s)")
    print(f"   Selected crossing closest to price (${underlying_price:.2f}):")
    print(f"   → Zero Gamma: ${best_crossing['zero_gamma']:.2f}")
    print(f"   → Distance: ${best_crossing['distance']:.2f}")
    print(f"   → Between ${best_crossing['strike1']:.2f} and ${best_crossing['strike2']:.2f}")
    
    return best_crossing['zero_gamma']
...
def convert_to_nq(levels, data):
    ...
    """
    print("\n" + "="*60)
    print("CONVERTING TO NQ LEVELS - v2.4 (FIXED)")
    print("="*60)
...
def save_results(levels, nq_levels, ratio):
    ...
    output = []
    output.append("="*60)
    output.append("GEX LEVELS OUTPUT - v2.4 (FIXED ZERO GAMMA)")
    output.append("="*60)
    ...
    try:
        with open("gex_levels_output_v2.4.txt", "w", encoding='utf-8') as f:
            f.write('\n'.join(output))
        print(f"\n💾 Results saved to 'gex_levels_output_v2.4.txt'")
    except:
